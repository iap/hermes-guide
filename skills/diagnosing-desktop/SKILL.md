...
| "A previous update left the desktop bundle incomplete" | Interrupted update (file locks) left a torn renderer bundle | Rebuild — detection is automatic (the stamp hashes intact *source*, so torn output forces a rebuild regardless) |
